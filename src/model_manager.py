...
class ImageUtils:
    ...
    @staticmethod
    def annotate_image(image, text, position=(10, 30), font_scale=1, color=(0, 255, 245), thickness=2):
...
class OptimizedYOLOModel(BaseModel):
    def __init__(self, model_path, conf=0.5, iou=0.45, img_size=320, device=None, half_precision=True):
...
class OptimizedYOLOV5Model(BaseModel):
    def __init__(self, model_path, conf=0.1, iou=0.45, img_size=320, device=None, half_precision=True):
        super().__init__(device)
...
class OptimizedHuggingFaceClassificationModel(BaseModel):
    def __init__(self, model_name, device=None):
        # Note: No conf, iou parameters for classification models
        super().__init__(device)
    ...
    def predict(self, input_data, annotate=True):
        ...
        if annotate:
            annotated_image = ImageUtils.annotate_image(image=image, text=predicted_label)
...
class OptimizedImageCaptioningModel(BaseModel):
    def __init__(self, model_name, device=None):
        # Note: No conf, iou parameters for captioning models
        super().__init__(device)
        ...
        self.model.eval()
        print(f"Loaded captioning model: {model_name}")
...
class SmartModelFactory:
    ...
    def create_model(model_type, model_path, **kwargs):
        """Smart factory that only passes relevant parameters to each model type"""
        if model_type == "yolo":
            # Only pass detection-specific parameters
            detection_params = {k: v for k, v in kwargs.items() 
                              if k in ['conf', 'iou', 'img_size', 'half_precision', 'device']}
            return OptimizedYOLOModel(model_path=model_path, **detection_params)
        
        elif model_type == "yolov5":
            detection_params = {k: v for k, v in kwargs.items() 
                              if k in ['conf', 'iou', 'img_size', 'half_precision', 'device']}
            return OptimizedYOLOV5Model(model_path=model_path, **detection_params)
        
        elif model_type == "classification":
            # Only pass device parameter, ignore conf/iou for classification
            classification_params = {k: v for k, v in kwargs.items() 
                                   if k in ['device']}
            return OptimizedHuggingFaceClassificationModel(model_name=model_path, **classification_params)
        
        elif model_type == "captioning":
            # Only pass device parameter, ignore conf/iou for captioning
            captioning_params = {k: v for k, v in kwargs.items() 
                               if k in ['device']}
            return OptimizedImageCaptioningModel(model_name=model_path, **captioning_params)
...
class HighPerformanceModelManager:
    ...
    def load_model(self, model_type, model_name, **kwargs):
        try:
            ...
            model = SmartModelFactory.create_model(model_type, model_name, **kwargs)
    ...
    def predict(self, image, **kwargs):
        ...
        try:
            # Only update detection-specific parameters for detection models
